...
class RotaryEmbedding(nn.Module):
    ...
    def apply_rope(self, pos, t):
        ...
        """
        # Reshape pos to broadcast over batch and num_heads: [1, seq_len, 1, head_dim]
...
class DiTBlock(nn.Module):
    ...
    def forward(self, x, cond, rotary_pos, padding_audio=None):
        ...
        x = self.norm1(x) * (1 + scale_msa[:, None]) + shift_msa[:, None]

        # Project q, k, v manually so RoPE can be applied before attention.
        d_model = x.size(-1)
        ...
        attn_out = attn_out.permute(0, 2, 1, 3).reshape(batch_size, seq_len, d_model)
        x = gate_msa[:, None] * attn_out + residual

        # ===== Gated MLP =====
        residual = x
        x = self.norm2(x) * (1 + scale_mlp[:, None]) + shift_mlp[:, None]
        mlp_out = self.mlp(x)
